...
def update_locations():
    for i in game.get_enemy_living_pirates():
        ...
        enemy_movement_vector[i.unique_id] = enemy_current_location[i.unique_id].subtract(enemy_previous_location[i.unique_id])
...
def approximated_enemy_role(enemy_ships):
    ...
    dis_to_selected_target = lambda enemy_ship, target : enemy_ship.get_location().towards(enemy_ship.get_location().add(enemy_movement_vector[enemy_ship.unique_id]), enemy_ship.distance(target))
...
def approximated_desination():
	dis_threshold = 5
	for enemy_ship in game.get_enemy_living_pirates:
		has_found_dest = False
		for obj_loc in allObjectsLocations:
			# checks what happens if it keeps on moving in its current direction:
			if enemy_ship.get_location().towards(enemy_ship.get_location().add(enemy_movement_vector[enemy_ship.unique_id]), enemy_ship.get_location().distance(obj_loc))<dis_threshold:
				approximated_desination[enemy_ship.unique_id] = obj_loc
		if has_found_dest == False:
			approximated_desination[enemy_ship.unique_id] = enemy_ship.get_location()
			
			
def loc_to_obj(location):
	#returns the object at the current location:
	all_Objects_but_mines = game.get_enemy_living_pirates().extend(game.get_all_motherships.extend(game.get_my_living_pirates()))
	mine_Objects = game.get_all_capsules
	for obj in all_Objects_but_mines:
		if obj.get_location() == location:
			return obj
	for obj in mine_Objects:
		if obj.initial_location == location:
			return obj
